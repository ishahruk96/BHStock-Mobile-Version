...
const CREATE_URL = "http://devmystock.byteheart.com/Products/Create";
const EDIT_URL = "http://devmystock.byteheart.com/Products/Edit";
const DELETE_URL = "http://devmystock.byteheart.com/Products/Delete";
const UNIT_TYPES_URL = "http://devmystock.byteheart.com/Products/GetUnitTypes";
    ...
    <View style={styles.modalOverlay}>
      <View style={styles.modalContent}>
        ...
        <View style={styles.modalButtons}>
          <TouchableOpacity style={[styles.modalBtn, styles.resetModalBtn]} onPress={onReset}>
            <MaterialIcons name="refresh" size={20} color="#fff" />
            <ThemedText style={styles.modalBtnText}>Reset All</ThemedText>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.modalBtn, styles.applyModalBtn]} onPress={onApply}>
        ...
        >
          <View style={styles.inputGroup}>
            ...
            <TextInput
              ...
              onChangeText={(v) => onInputChange("ProductName", v)}
          ...
          <View style={styles.inputGroup}>
            ...
            <TextInput
              ...
              onChangeText={(v) => onInputChange("Category", v)}
              placeholderTextColor="#999"
            />
          </View>

          <View style={styles.inputGroup}>
            <ThemedText style={styles.inputLabel}>Buy Price *</ThemedText>
            <TextInput
              style={styles.input}
              placeholder="Enter buy price"
              value={newProduct.UnitPrice}
              onChangeText={(v) => onInputChange("UnitPrice", v)}
          ...
          <View style={styles.inputGroup}>
            <ThemedText style={styles.inputLabel}>Sales Price *</ThemedText>
            <TextInput
              style={styles.input}
              placeholder="Enter sales price"
              value={newProduct.SalesValue}
              onChangeText={(v) => onInputChange("SalesValue", v)}
              keyboardType="numeric"
              placeholderTextColor="#999"
            />
          </View>
          ...
          <View style={styles.inputGroup}>
            <ThemedText style={styles.inputLabel}>Unit Type *</ThemedText>
            <ScrollView
              horizontal
              showsHorizontalScrollIndicator={false}
              style={styles.unitTypeScroll}
              keyboardShouldPersistTaps="handled"
            >
              {unitTypes.map((type) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.unitTypeChip,
                    newProduct.UnitType === type.name && styles.unitTypeChipActive
                  ]}
                  onPress={() => onInputChange("UnitType", type.name)}
                >
                  <ThemedText style={[
                    styles.unitTypeChipText,
                    newProduct.UnitType === type.name && styles.unitTypeChipTextActive
                  ]}>
                    {type.name}
                  </ThemedText>
                </TouchableOpacity>
          ...
          <View style={styles.inputGroup}>
            <ThemedText style={styles.inputLabel}>Current Stock *</ThemedText>
            <TextInput
              style={styles.input}
              placeholder="Enter current stock quantity"
              value={newProduct.CurrentStock}
              onChangeText={(v) => onInputChange("CurrentStock", v)}
              keyboardType="numeric"
              placeholderTextColor="#999"
            />
          </View>
          ...
          <View style={styles.inputGroup}>
            <ThemedText style={styles.inputLabel}>Reorder Level *</ThemedText>
            <TextInput
              style={styles.input}
              placeholder="Enter reorder level"
              value={newProduct.ReorderLevel}
              onChangeText={(v) => onInputChange("ReorderLevel", v)}
              keyboardType="numeric"
              placeholderTextColor="#999"
            />
          </View>
          ...
          <View style={styles.profitInfoContainer}>
            <ThemedText style={styles.profitInfoTitle}>Profit Information</ThemedText>
            <View style={styles.profitInfoRow}>
              <View style={styles.profitInfoItem}>
                <ThemedText style={styles.profitInfoLabel}>Profit Per Unit</ThemedText>
                <ThemedText style={styles.profitInfoValue}>
                  ৳{newProduct.ProfitPerUnit || "0.00"}
                </ThemedText>
              </View>
              <View style={styles.profitInfoItem}>
                <ThemedText style={styles.profitInfoLabel}>Profit Percentage</ThemedText>
                <ThemedText style={styles.profitInfoValue}>
                  {newProduct.ProfitPercentage || "0.00"}%
                </ThemedText>
              </View>
            </View>
          </View>
            ...
            >
              {STATUSES.map((status) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.statusChip,
                    newProduct.Status === status && styles.statusChipActive
                  ]}
                  onPress={() => onInputChange("Status", status)}
                >
                  <ThemedText style={[
                    styles.statusChipText,
                    newProduct.Status === status && styles.statusChipTextActive
                  ]}>
                    {status}
                  </ThemedText>
                </TouchableOpacity>
          ...
          >
            {isSubmitting ? (
              ...
              <>
                ...
                <ThemedText style={styles.modalBtnText}>Save Product</ThemedText>
              </>
            )}
          </TouchableOpacity>
        </View>
            ...
            >
              {unitTypes.map((type) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.unitTypeChip,
                    editProduct.UnitType === type.name && styles.unitTypeChipActive
                  ]}
                  onPress={() => onInputChange("UnitType", type.name)}
                >
                  <ThemedText style={[
                    styles.unitTypeChipText,
                    editProduct.UnitType === type.name && styles.unitTypeChipTextActive
                  ]}>
                    {type.name}
                  </ThemedText>
                </TouchableOpacity>
          ...
          <View style={styles.profitInfoContainer}>
            ...
            <View style={styles.profitInfoRow}>
              <View style={styles.profitInfoItem}>
                <ThemedText style={styles.profitInfoLabel}>Profit Per Unit</ThemedText>
                <ThemedText style={styles.profitInfoValue}>
                  ৳{editProduct.ProfitPerUnit || "0.00"}
                </ThemedText>
              </View>
              <View style={styles.profitInfoItem}>
                <ThemedText style={styles.profitInfoLabel}>Profit Percentage</ThemedText>
                <ThemedText style={styles.profitInfoValue}>
                  {editProduct.ProfitPercentage || "0.00"}%
                </ThemedText>
              </View>
            </View>
          </View>
            ...
            >
              {STATUSES.map((status) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.statusChip,
                    editProduct.Status === status && styles.statusChipActive
                  ]}
                  onPress={() => onInputChange("Status", status)}
                >
                  <ThemedText style={[
                    styles.statusChipText,
                    editProduct.Status === status && styles.statusChipTextActive
                  ]}>
                    {status}
                  </ThemedText>
                </TouchableOpacity>
        ...
        <View style={styles.modalButtons}>
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <MaterialIcons name="save" size={20} color="#fff" />
                <ThemedText style={styles.modalBtnText}>Update Product</ThemedText>
              </>
            )}
          </TouchableOpacity>
        </View>
...
const ProductCard = ({ item, onEdit, onDelete }: ProductCardProps) => {
  const totalProfit   = item.ProfitPerUnit * item.CurrentStock;
  const isLowStock    = item.CurrentStock <= item.ReorderLevel;
  const stockStatus   = isLowStock && item.CurrentStock > 0
    ? "Low Stock"
    : item.CurrentStock === 0
      ? "Out of Stock"
      : "Available";
  const statusColor       = getStatusColor(stockStatus);
  const profitPercentage  = item.CurrentStock > 0
    ? ((item.ProfitPerUnit / item.UnitPrice) * 100).toFixed(1)
    : "0";

  return (
    <TouchableOpacity activeOpacity={0.9} onPress={() => onEdit(item)}>
...
const EMPTY_EDIT_PRODUCT: EditProduct = {
  ProductId: 0,
  ProductName: "",
  Category: "",
  UnitPrice: "",
...
export default function ProductsScreen() {
  ...
  const [editProduct, setEditProduct]         = useState<EditProduct>(EMPTY_EDIT_PRODUCT);
  const [isSubmitting, setIsSubmitting]       = useState(false);

  const fadeAnim = useRef(new Animated.Value(0)).current;

  useEffect(() => { fetchProducts(); fetchUnitTypes(); }, []);

  useEffect(() => { applyFilters(); }, [searchQuery, selectedCategory, selectedStatus, products]);
  ...
  const fetchProducts = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        setCategories([...new Set(data.map((p: Product) => p.Category).filter(Boolean))]);
      } else {
  ...
  const fetchUnitTypes = async () => {
    try {
      const response = await fetch(UNIT_TYPES_URL, {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${API_KEY}`,
          "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        let arr: any[] = Array.isArray(data)
          ? data
          : data.data || data.items || Object.values(data).filter(i => i && typeof i === 'object');
        setUnitTypes(arr.map((item: any, i: number) => ({
          id: item.id || item.Id || item.unitTypeId || item.UnitTypeId || i,
          name: item.name || item.Name || item.unitTypeName || item.UnitTypeName || String(item)
        })));
      } else {
  ...
  // Handle input change for Add Product
  const handleAddInputChange = useCallback((field: string, value: string) => {
    setNewProduct(prev => {
      const updated = { ...prev, [field]: value };

      const buyPrice  = parseFloat(field === "UnitPrice"  ? value : prev.UnitPrice)  || 0;
      const salePrice = parseFloat(field === "SalesValue" ? value : prev.SalesValue) || 0;
      if (field === "UnitPrice" || field === "SalesValue") {
        const profitPerUnit       = salePrice - buyPrice;
        const profitPercentage    = buyPrice > 0 ? (profitPerUnit / buyPrice) * 100 : 0;
        updated.ProfitPerUnit     = profitPerUnit.toFixed(2);
  ...
  // Handle input change for Edit Product
  ...
  const validateAddForm = () => {
    if (!newProduct.ProductName.trim())                              { Alert.alert("Validation Error", "Product Name is required");   return false; }
    if (!newProduct.Category.trim())                                 { Alert.alert("Validation Error", "Category is required");        return false; }
    if (!newProduct.UnitPrice   || parseFloat(newProduct.UnitPrice)   <= 0) { Alert.alert("Validation Error", "Valid Buy Price is required");  return false; }
    if (!newProduct.SalesValue  || parseFloat(newProduct.SalesValue)  <= 0) { Alert.alert("Validation Error", "Valid Sales Price is required"); return false; }
    if (!newProduct.UnitType)                                        { Alert.alert("Validation Error", "Unit Type is required");       return false; }
    if (!newProduct.CurrentStock || parseFloat(newProduct.CurrentStock) < 0) { Alert.alert("Validation Error", "Valid Stock quantity is required"); return false; }
    if (!newProduct.ReorderLevel || parseFloat(newProduct.ReorderLevel) < 0) { Alert.alert("Validation Error", "Valid Reorder Level is required"); return false; }
    return true;
  };

  const validateEditForm = () => {
    if (!editProduct.ProductName.trim())                              { Alert.alert("Validation Error", "Product Name is required");   return false; }
    if (!editProduct.Category.trim())                                 { Alert.alert("Validation Error", "Category is required");        return false; }
    if (!editProduct.UnitPrice   || parseFloat(editProduct.UnitPrice)   <= 0) { Alert.alert("Validation Error", "Valid Buy Price is required");  return false; }
    if (!editProduct.SalesValue  || parseFloat(editProduct.SalesValue)  <= 0) { Alert.alert("Validation Error", "Valid Sales Price is required"); return false; }
    if (!editProduct.UnitType)                                        { Alert.alert("Validation Error", "Unit Type is required");       return false; }
    if (!editProduct.CurrentStock || parseFloat(editProduct.CurrentStock) < 0) { Alert.alert("Validation Error", "Valid Stock quantity is required"); return false; }
    if (!editProduct.ReorderLevel || parseFloat(editProduct.ReorderLevel) < 0) { Alert.alert("Validation Error", "Valid Reorder Level is required"); return false; }
    return true;
  };
    ...
    try {
      const response = await fetch(CREATE_URL, {
        ...
        body: JSON.stringify({
          ...
          SalesValue:       parseFloat(newProduct.SalesValue),
          ProfitPerUnit:    parseFloat(newProduct.ProfitPerUnit),
          ProfitPercentage: parseFloat(newProduct.ProfitPercentage),
          CurrentStock:     parseFloat(newProduct.CurrentStock),
      ...
      await fetchProducts();
      showSuccess(`${newProduct.ProductName} has been created successfully!`);
    } catch (err: any) {
      Alert.alert("Error", `Failed to create product: ${err.message}`);
    } finally {
    ...
    try {
      const response = await fetch(EDIT_URL, {
        ...
        body: JSON.stringify({
          ...
          SalesValue:       parseFloat(editProduct.SalesValue),
          ProfitPerUnit:    parseFloat(editProduct.ProfitPerUnit),
          ProfitPercentage: parseFloat(editProduct.ProfitPercentage),
          CurrentStock:     parseFloat(editProduct.CurrentStock),
          ReorderLevel:     parseFloat(editProduct.ReorderLevel),
          UnitType:         editProduct.UnitType,
          Status:           editProduct.Status,
          OrganizationId:   editProduct.OrganizationId,
        })
      });
      if (!response.ok) {
        const errorText = await response.text();
      ...
      await fetchProducts();
      showSuccess(`${editProduct.ProductName} has been updated successfully!`);
    } catch (err: any) {
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            setIsSubmitting(true);
            try {
              const response = await fetch(`${DELETE_URL}?id=${product.ProductId}`, {
                method: "DELETE",
                headers: {
                  "Authorization": `Bearer ${API_KEY}`,
                  "Content-Type": "application/json",
              ...
              await fetchProducts();
              showSuccess(`${product.ProductName} has been deleted successfully!`);
            } catch (err: any) {
  ...
  const handleEdit = useCallback((product: Product) => {
    setEditProduct({
      ...
      SalesValue: product.SalesValue.toString(),
  ...
  const applyFilters = useCallback(() => {
    ...
    if (searchQuery)       filtered = filtered.filter(p => p.ProductName?.toLowerCase().includes(searchQuery.toLowerCase()) || p.Category?.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const renderItem = useCallback(({ item }: { item: Product }) => (
    <ProductCard item={item} onEdit={handleEdit} onDelete={handleDeleteProduct} />
  ), [handleEdit]);

  const keyExtractor = useCallback((item: Product) => item.ProductId.toString(), []);
    ...
    <View style={styles.container}>
      ...
      {showSuccessMessage && (
        <Animated.View style={[styles.successToast, { opacity: fadeAnim }]}>
          ...
          <ThemedText style={styles.successToastText}>{successMessage}</ThemedText>
        </Animated.View>
      )}
...
const styles = StyleSheet.create({
  ...
  modalContent: { backgroundColor: '#fff', borderTopLeftRadius: 24, borderTopRightRadius: 24, maxHeight: '95%', padding: 20 },
  ...
  cancelModalBtn: { backgroundColor: '#dc3545' },
  saveModalBtn: { backgroundColor: '#28a745' },
  ...
  inputLabel: { fontSize: 14, fontWeight: '500', marginBottom: 8, color: '#555' },
  input: { borderWidth: 1, borderColor: '#ddd', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 10, fontSize: 14, backgroundColor: '#fafafa' },
  unitTypeScroll: { flexDirection: 'row', maxHeight: 50 },
  unitTypeChip: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: '#f0f0f0', marginRight: 8, borderWidth: 1, borderColor: '#e0e0e0' },
  unitTypeChipActive: { backgroundColor: '#28a745', borderColor: '#28a745' },
  unitTypeChipText: { fontSize: 13, color: '#666' },
  unitTypeChipTextActive: { color: '#fff' },
  statusChip: { paddingHorizontal: 16, paddingVertical: 8, borderRadius: 20, backgroundColor: '#f0f0f0', marginRight: 8, borderWidth: 1, borderColor: '#e0e0e0' },
  statusChipActive: { backgroundColor: '#28a745', borderColor: '#28a745' },
  statusChipText: { fontSize: 13, color: '#666' },
  statusChipTextActive: { color: '#fff' },
  profitInfoContainer: { backgroundColor: '#f8f9fa', padding: 16, borderRadius: 12, marginBottom: 16 },
  profitInfoTitle: { fontSize: 14, fontWeight: 'bold', color: '#333', marginBottom: 12 },
  profitInfoRow: { flexDirection: 'row', justifyContent: 'space-between' },
  profitInfoItem: { flex: 1, alignItems: 'center' },
  profitInfoLabel: { fontSize: 12, color: '#666', marginBottom: 4 },
  profitInfoValue: { fontSize: 16, fontWeight: 'bold', color: '#28a745' },

  successToast: { position: 'absolute', top: Platform.OS === 'ios' ? 60 : 40, left: 20, right: 20, backgroundColor: '#28a745', borderRadius: 12, padding: 16, flexDirection: 'row', alignItems: 'center', gap: 12, zIndex: 999, elevation: 6, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4 },
  successToastText: { color: '#fff', fontSize: 14, fontWeight: '600', flex: 1 },
});